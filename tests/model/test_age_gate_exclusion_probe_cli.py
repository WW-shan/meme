...
def _load_cli():
    path = Path(__file__).resolve().parents[2] / "scripts" / "probe_age_gate_exclusion.py"
    spec = importlib.util.spec_from_file_location("probe_age_gate_exclusion", path)
...
class TestAgeGateExclusionProbeCli(unittest.TestCase):
    def test_main_writes_probe_report_from_multiple_inputs(self):
        cli = _load_cli()
        with tempfile.TemporaryDirectory(dir="data/replay_reports") as tmpdir:
            ...
            output = Path(tmpdir) / "age_gate.json"
    ...
    def test_main_refuses_invalid_age_cuts(self):
        cli = _load_cli()
        with tempfile.TemporaryDirectory(dir="data/replay_reports") as tmpdir:
            first = Path(tmpdir) / "first.json"
            second = Path(tmpdir) / "second.json"
            ...
            self.assertEqual(result, 2)
            self.assertIn("invalid --age-cuts", stderr.getvalue())

    def test_main_refuses_invalid_volume_cuts(self):
        cli = _load_cli()
        with tempfile.TemporaryDirectory(dir="data/replay_reports") as tmpdir:
            first = Path(tmpdir) / "first.json"
            second = Path(tmpdir) / "second.json"
            ...
            self.assertEqual(result, 2)
            self.assertIn("invalid --volume-cuts", stderr.getvalue())

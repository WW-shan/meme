...
def _load_cli():
    path = Path(__file__).resolve().parents[2] / "scripts" / "probe_airtag_class_exclusion.py"
    spec = importlib.util.spec_from_file_location("probe_airtag_class_exclusion", path)
...
class TestAirTagClassExclusionProbeCli(unittest.TestCase):
    def test_main_writes_probe_report_from_multiple_inputs(self):
        ...
        with tempfile.TemporaryDirectory(dir="data/replay_reports") as tmpdir:
            ...
            output = Path(tmpdir) / "airtag.json"

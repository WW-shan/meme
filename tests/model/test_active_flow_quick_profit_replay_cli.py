...
def _load_cli():
    path = Path(__file__).resolve().parents[2] / "scripts" / "run_active_flow_quick_profit_replay.py"
    spec = importlib.util.spec_from_file_location("run_active_flow_quick_profit_replay", path)
...
class TestActiveFlowQuickProfitReplayCli(unittest.TestCase):
    def test_parse_args_defaults_to_active_flow_report_and_keeps_live_risk(self):
        cli = _load_cli()

        args = cli.parse_args([])

        self.assertEqual(args.model_dir, "data/models/20260519_v95_v84_selective_nearmiss_gate")
        self.assertEqual(args.output, "data/replay_reports/active_flow_quick_profit_replay_20260522_v95.json")
    ...
    def test_parse_args_respects_abbreviated_output_flag_from_base_parser(self):
        cli = _load_cli()

        args = cli.parse_args(["--out", "custom-active-flow.json"])

        self.assertEqual(args.output, "custom-active-flow.json")

    def test_candidate_grid_is_three_cell_total_buys_proxy_only(self):
        cli = _load_cli()
        ...
        self.assertEqual(
            [candidate["buy_quick_profit_overlay_min_total_buys"] for candidate in candidates],
            [6.0, 10.0, 14.0],
        )
        for candidate in candidates:
            self.assertEqual(candidate["buy_quick_profit_overlay_min_prob"], 0.985)
            ...
            self.assertEqual(candidate["buy_quick_profit_overlay_min_entry_volume_30s"], 1.25)
            self.assertEqual(candidate["buy_quick_profit_overlay_min_entry_price_volatility"], 0.08)
            ...
            self.assertNotIn("buy_quick_profit_overlay_max_buy_sell_overlap_ratio_60s", candidate)
            self.assertNotIn("buy_quick_profit_overlay_max_recent_seller_reentry_ratio_30s", candidate)

    def test_main_uses_active_flow_grid_and_preloaded_samples(self):
        cli = _load_cli()
        calls = []
        sample_loads = []
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            output_path = Path(tmpdir) / "active_flow_report.json"
        ...
        self.assertEqual(calls[1]["overrides"]["buy_quick_profit_overlay_min_total_buys"], 10.0)
        self.assertNotIn("buy_quick_profit_overlay_min_prob", calls[2]["overrides"])
        self.assertEqual(calls[3]["overrides"]["buy_quick_profit_overlay_min_total_buys"], 10.0)

...
A newer 2026-05-18 offline candidate now ranks ahead of the v67 entry-protection sweep, but it has not replaced the live baseline until the live switch procedure is explicitly run. The candidate is `data/models/20260518_v83_targethit60_riskadjusted_entryv` with `buy_threshold=0.98`, `entry_ranking_mode=entry_value`, `min_entry_score=35`, `min_entry_volume_30s=1.5`, `min_entry_price_volatility=0.1`, `entry_price_protection_pct=0.18`, and the same 10% sizing plus hold75 exit profile. Its strict final metrics were `304.4618%` net return, `0.01825727` BNB profit, `70.4918%` win rate, `-6.6565%` max drawdown, `21.3442%` walk-forward worst return, `-14.3655%` walk-forward worst drawdown, `99.3919%` harsh-friction stress return, and `75.6030%` harsh-execution stress return over `61` trades. It beats current v67 on return, profit, win rate, max drawdown, WF worst drawdown, and stress replay; its WF worst return is positive but slightly below v67's `25.2537%`, so keep monitoring live evidence after any later switch.

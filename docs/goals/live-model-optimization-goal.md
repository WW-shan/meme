...
A newer 2026-05-18 offline candidate now ranks ahead of the v67 entry-protection sweep, but it has not replaced the live baseline until the live switch procedure is explicitly run. The candidate is `data/models/20260518_v83_targethit60_riskadjusted_entryv` with `buy_threshold=0.98`, `entry_ranking_mode=entry_value`, `min_entry_score=35`, `min_entry_volume_30s=1.5`, `min_entry_price_volatility=0.1`, `entry_price_protection_pct=0.18`, `stop_loss=-0.18`, and the same 10% sizing plus hold75 exit profile. Its strict final metrics were `307.6195%` net return, `0.01844662` BNB profit, `70.4918%` win rate, `-6.5848%` max drawdown, `23.7401%` walk-forward worst return, `-14.3655%` walk-forward worst drawdown, `101.3053%` harsh-friction stress return, and `76.9625%` harsh-execution stress return over `61` trades. It beats current v67 on return, profit, win rate, max drawdown, WF worst drawdown, and stress replay; its WF worst return is positive and close to v67's `25.2537%`, so keep monitoring live evidence after any later switch.

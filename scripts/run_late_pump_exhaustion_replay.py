...
DEFAULT_OUTPUT = "data/replay_reports/late_pump_exhaustion_replay_20260519_v95.json"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run a bounded late-pump exhaustion veto replay grid")
...
def candidate_grid():
    min_ages = [15, 120]
    windows = [30]
    extension_floors = [1.0]
    peak_drawdown_floors = [0.45, 0.55]
    volume_floors = [2.0, 3.0]
    volatility_floors = [0.18, 0.22]
    for values in itertools.product(
        ...
        min_age, window, extension_floor, peak_drawdown_floor, volume_floor, volatility_floor = values
        yield {
            "buy_late_pump_veto_min_age_seconds": min_age,
            "buy_late_pump_veto_extension_window_seconds": window,
            "buy_late_pump_veto_min_price_extension_pct": extension_floor,
            "buy_late_pump_veto_min_drawdown_from_peak_pct": peak_drawdown_floor,
            "buy_late_pump_veto_min_entry_volume_30s": volume_floor,
            "buy_late_pump_veto_min_entry_price_volatility": volatility_floor,
        }
...
def _summary(evaluation):
    ...
    return {
        ...
        "late_pump_veto_reject_count": int(evaluation.get("late_pump_veto_reject_count", 0) or 0),
    }
...
def _acceptance_gate():
    return {
        ...
        "requires_late_pump_veto_rejections": True,
    }
...
def _gate_details(candidate_summary, baseline_summary):
    ...
    return {
        ...
        "late_pump_veto_reject_count": candidate_summary["late_pump_veto_reject_count"] > 0,
    }
...
def main(argv=None):
    ...
    report = {
        ...
        "hypothesis": "Late pump exhaustion veto skips TSG-like high-score chase entries without increasing live risk.",

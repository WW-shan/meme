...
DEFAULT_OUTPUT_JSON = "data/replay_reports/buy_not_ready_outcomes.json"
DEFAULT_OUTPUT_MD = "data/replay_reports/buy_not_ready_outcomes.md"


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Build a read-only BUY_NOT_READY outcome report")
    ...
    parser.add_argument("--reason-contains", default="Unsupported quote asset")
    parser.add_argument("--include-all-reasons", action="store_true")
    parser.add_argument("--max-hold-seconds", type=float, default=560.0)
    parser.add_argument("--horizon-seconds", type=float, default=10800.0)
    parser.add_argument("--min-support", type=int, default=3)
    parser.add_argument("--max-sample", type=int, default=100, help="0 emits all scored events")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args(argv)
    if args.recent_lifecycle_files < 0:
        parser.error("--recent-lifecycle-files must be non-negative")
    if args.max_hold_seconds <= 0.0:
        parser.error("--max-hold-seconds must be positive")
    if args.horizon_seconds < args.max_hold_seconds:
        parser.error("--horizon-seconds must be at least --max-hold-seconds")
    if args.min_support < 1:
        parser.error("--min-support must be at least 1")
    if args.max_sample < 0:
        parser.error("--max-sample must be non-negative")
    return args
...
def main(argv=None) -> int:
    ...
    probe = importlib.import_module("src.pipeline.buy_not_ready_probe")
    ...
    try:
        ...
        reason_contains = None if args.include_all_reasons else args.reason_contains
        report = probe.build_buy_not_ready_outcome_report(

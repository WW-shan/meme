...
from src.pipeline import action_policy_reward_probe as probe
...
def _default_output() -> str:
    ...
    return f"data/replay_reports/action_policy_reward_probe_{stamp}.json"


def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Build a read-only action-policy direct reward replay probe",
    )
    parser.add_argument("--train-rejected-report", action="append", required=True)
    parser.add_argument("--train-accepted-report", action="append", required=True)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        report = probe.build_action_policy_reward_report(

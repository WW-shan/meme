...
MAX_GRID_CANDIDATES = 64


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run a bounded 60s flow-toxicity abstention replay grid")
...
def _flow_condition_candidates():
    yield {"buy_flow_abstention_min_buy_sell_overlap_ratio_60s": 0.875}
    yield {"buy_flow_abstention_max_buy_sell_ratio_60s": 0.005}
    yield {"buy_flow_abstention_min_sell_pressure_60s": 0.995}
    yield {"buy_flow_abstention_max_signed_imbalance_60s": -0.995}
...
def candidate_grid():
    ...
    if len(candidates) > MAX_GRID_CANDIDATES:
        raise RuntimeError(f"flow toxicity grid has {len(candidates)} candidates; max is {MAX_GRID_CANDIDATES}")
...
def main(argv=None):
    ...
    base_overrides = base._base_overrides(args)
    ...
    if not candidates:
        raise SystemExit("flow toxicity meta-gate replay grid produced no candidates")
    ...
    report = {
        ...
        "hypothesis": (
            "A decision-time 60s flow-toxicity veto skips adverse-flow or high-overlap "

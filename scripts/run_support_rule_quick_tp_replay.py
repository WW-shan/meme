...
DEFAULT_OUTPUT = "data/replay_reports/support_rule_quick_tp_replay_20260522_v95.json"
...
def parse_args(argv=None):
    ...
    args = _base.parse_args(filtered_args)
    args.validation_only = bool(validation_only)
...
def candidate_grid():
    # Replay the strongest decision-time support-rule shapes from the wide-window
    # support gate without reintroducing the previously rejected total_buys proxy.
    rule_shapes = [{
    ...
    for shape, take_profit, max_hold in itertools.product(rule_shapes, take_profits, max_holds):
        yield {
            **shape,

...
DEFAULT_OUTPUT = "data/replay_reports/ultrashort_runner_replay_20260521_v95.json"


def parse_args(argv=None):
    argv_list = list(sys.argv[1:] if argv is None else argv)
    args = _base.parse_args(argv_list)
    if not any(arg == "--output" or str(arg).startswith("--output=") for arg in argv_list):
...
def candidate_grid():
    ...
    pred_return_floors = [10.0]
    volume_floors = [1.25, 1.35]

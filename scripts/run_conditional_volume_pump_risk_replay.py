...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run conditional low-volume runner plus pump-risk veto replay grid")
...
def candidate_grid():
    prob_floors = [0.988, 0.99]
    volume_mins = [0.95, 1.15]
    volatility_floors = [0.08]
    max_ages = [60]
    extension_floors = [0.8, 1.0]
    for min_prob, min_volume, min_volatility, max_age, extension_floor in itertools.product(
        prob_floors,
        volume_mins,
        volatility_floors,
        max_ages,
        ...
        yield {
            ...
            "buy_late_pump_veto_min_entry_price_volatility": min_volatility,
        }
...
def _summary(evaluation):
    ...
    return {
        ...
        "near_threshold_entry_count": int(evaluation.get("near_threshold_entry_count", 0) or 0),
    }
...
def _acceptance_gate():
    return {
        ...
        "requires_near_threshold_rescue_disabled": True,
    }
...
def _gate_details(candidate_summary, baseline_summary):
    ...
    return {
        ...
        "near_threshold_entry_count": candidate_summary["near_threshold_entry_count"] == 0,
    }
...
def main(argv=None):
    ...
    print(
        f"final confirmation candidate {validation_selected['candidate_index']}",
        flush=True,
    )
    ...
    report = {
        ...
        "live_trigger": {
            "problem": "v95 live near-threshold rescue has been negative while vertical pump entries show high slippage risk",
            "examples": ["FENGSHUI", "挠头", "EX677", "FourPass", "GNGN"],
        },
        "baseline": {
            "split": "validation",

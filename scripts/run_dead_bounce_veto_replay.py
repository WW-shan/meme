...
LIVE_INITIAL_EQUITY_BNB = 0.003878251642954092
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run a bounded dead-bounce veto replay grid")
...
def candidate_grid():
    age_ceilings = [15.0, 30.0]
    peak_drawdowns = [0.60, 0.70]
    creator_sell_floors = [1.0, 2.0]
    buy_pressure_ceilings = [0.30, 0.35]
    volume_floors = [1.5, 2.0]
    volatility_floors = [0.10, 0.18]

    # Cross the primary shape/sell-pressure dimensions and cycle the liquidity
    # floors deterministically to keep the sealed grid below 64 candidates.
    for age_index, drawdown_index, creator_index, pressure_index in itertools.product(
        range(len(age_ceilings)),
        range(len(peak_drawdowns)),
        range(len(creator_sell_floors)),
        range(len(buy_pressure_ceilings)),
    ):
        yield {
            "buy_dead_bounce_veto_max_age_seconds": age_ceilings[age_index],
            "buy_dead_bounce_veto_min_peak_drawdown_pct": peak_drawdowns[drawdown_index],
            "buy_dead_bounce_veto_min_creator_sell_volume_bnb": creator_sell_floors[creator_index],
            "buy_dead_bounce_veto_max_buy_pressure": buy_pressure_ceilings[pressure_index],
            "buy_dead_bounce_veto_min_entry_volume_30s": volume_floors[
                (age_index + creator_index + pressure_index) % len(volume_floors)
            ],
            "buy_dead_bounce_veto_min_entry_price_volatility": volatility_floors[
                (drawdown_index + creator_index + pressure_index) % len(volatility_floors)
            ],
        }
...
def _summary(evaluation):
    ...
    return {
        ...
        "dead_bounce_veto_signal_count": _int_metric(evaluation, "dead_bounce_veto_signal_count"),
        "dead_bounce_veto_reject_count": _int_metric(evaluation, "dead_bounce_veto_reject_count"),
    }
...
def _acceptance_gate():
    return {
        ...
        "requires_dead_bounce_veto_rejections": True,
    }
...
def _gate_details(candidate_summary, baseline_summary):
    ...
    return {
        ...
        "dead_bounce_veto_reject_count": candidate_summary["dead_bounce_veto_reject_count"] > 0,
    }
...
def main(argv=None):
    ...
    print("RUN validation baseline", flush=True)
    ...
    print(f"RUN validation candidates count={len(grid)}", flush=True)
    for index, params in enumerate(grid):
        print(f"RUN validation candidate={index} params={json.dumps(params, sort_keys=True)}", flush=True)
        overrides = dict(base_overrides)
        overrides.update(params)
        report = _run_replay(args, split="validation", overrides=overrides)
    ...
    final_confirmation = {
        ...
        "skipped": True,
        "reason": "no_validation_candidate_passed_acceptance_gate",
    }
    if validation_selected is not None:
        print(f"RUN final baseline and selected_candidate={validation_selected['candidate_index']}", flush=True)
    ...
    report = {
        ...
        "hypothesis": (
            "Dead-bounce veto skips young v95 primary buys after peak crash plus "
            "creator or sell-pressure evidence without increasing live risk."
        ),
    ...
    selected = validation_selected or best_validation_raw_candidate
    print(
        ...
        "final_confirmation_passed={final_passed} candidates={count} output={output}".format(
            ...
            best=selected["summary"]["net_profit_bnb"],

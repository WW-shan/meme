...
DEFAULT_OUTPUT = "data/replay_reports/entry_slippage_risk_veto_replay_20260522_v95.json"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run a bounded entry-slippage risk veto replay grid")
    ...
    parser.add_argument("--max-candidates", type=_positive_int, default=None, help="Limit candidates for diagnostic runs")
    parser.add_argument(
        "--candidate-grid-json",
    ...
    parser.add_argument(
        "--confirm-best-raw",
        action="store_true",
        help="Run final confirmation for the best raw validation candidate even when it fails the strict acceptance gate",
    )
    parser.add_argument(
        "--write-selected-trade-delta",
        action="store_true",
        help="Attach paired trade-delta attribution for the selected validation/final candidate when final confirmation runs",
    )
...
def candidate_grid():
    min_ages = [15]
    windows = [30, 120]
    extension_floors = [1.0, 2.0]
    peak_drawdown_floors = [0.30, 0.45]
    recent_jump_floors = [0.10, 0.20]
    ...
    for values in itertools.product(
        ...
        windows,
        extension_floors,
        peak_drawdown_floors,
        recent_jump_floors,
        volume_floors,
        volatility_floors,
    ):
        min_age, window, extension_floor, peak_drawdown_floor, recent_jump_floor, volume_floor, volatility_floor = values
        yield {
            "buy_entry_slippage_risk_veto_min_age_seconds": min_age,
            "buy_entry_slippage_risk_veto_extension_window_seconds": window,
            "buy_entry_slippage_risk_veto_min_price_extension_pct": extension_floor,
            "buy_entry_slippage_risk_veto_min_drawdown_from_peak_pct": peak_drawdown_floor,
            "buy_entry_slippage_risk_veto_min_recent_jump_pct": recent_jump_floor,
            "buy_entry_slippage_risk_veto_min_entry_volume_30s": volume_floor,
            "buy_entry_slippage_risk_veto_min_entry_price_volatility": volatility_floor,
        }
...
def _summary(evaluation):
    ...
    return {
        ...
        "entry_slippage_risk_veto_reject_count": int(
            evaluation.get("entry_slippage_risk_veto_reject_count", 0) or 0
        ),
    }
...
def _acceptance_gate():
    return {
        ...
        "requires_entry_slippage_risk_veto_rejections": True,
    }
...
def _gate_details(candidate_summary, baseline_summary):
    ...
    return {
        ...
        "entry_slippage_risk_veto_reject_count": candidate_summary["entry_slippage_risk_veto_reject_count"] > 0,
    }
...
def main(argv=None):
    ...
    candidate_iter = iter(candidate_params_grid)
    if args.max_candidates is not None:
        candidate_iter = itertools.islice(candidate_iter, int(args.max_candidates))
    for index, params in enumerate(candidate_iter):
    ...
    should_confirm_final = best_validation_accepted is not None or bool(args.confirm_best_raw)
    if should_confirm_final:
    ...
    if bool(args.write_selected_trade_delta) and final_confirmation is not None:
    ...
    final_passed = bool(final_confirmation and final_confirmation["passes_acceptance_gate"])
    decision = "accept" if best_validation_accepted is not None and final_passed else "reject"
    report = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "model_dir": str(args.model_dir),
        "lifecycle_dir": str(args.lifecycle_dir),
        "hypothesis": "Entry slippage risk veto skips high extension/recent-jump candidates without increasing live risk.",
        "live_switch_evidence": False,
        "strict_assumptions": base_overrides,
        "candidate_limit": None if args.max_candidates is None else int(args.max_candidates),
        "candidate_grid": {
            "source": str(args.candidate_grid_json) if args.candidate_grid_json else "default",
            "candidate_count": len(candidate_params_grid),
        },
        "confirm_best_raw": bool(args.confirm_best_raw),

...
DEFAULT_OUTPUT = "data/replay_reports/competing_risk_quick_profit_replay_20260526_v95.json"
...
def candidate_grid():
    # Competing-risk quick-profit falsification: unlike older scalp grids, this
    # tests low/zero PredReturn candidates because the live fast-profit bucket is
    # often separated by event timing rather than entry-value magnitude.
    prob_floors = [0.985, 0.988]
    pred_return_floors = [0.0, 5.0]
    volume_floors = [1.25, 1.5]
    ...
    for values in itertools.product(
        ...
        min_prob, min_pred_return, min_volume, min_volatility, max_age, take_profit, max_hold = values
        yield {
            "buy_quick_profit_overlay_min_prob": min_prob,
            "buy_quick_profit_overlay_min_pred_return": min_pred_return,
            "buy_quick_profit_overlay_max_pred_return": 35.0,
            "buy_quick_profit_overlay_min_entry_volume_30s": min_volume,
            "buy_quick_profit_overlay_min_entry_price_volatility": min_volatility,
            "buy_quick_profit_overlay_max_age_seconds": max_age,
            "buy_quick_profit_overlay_take_profit_pct": take_profit,
            "buy_quick_profit_overlay_max_hold_seconds": max_hold,
        }
...
def _overrides_with_eval_samples(base_overrides, eval_samples):
    overrides = dict(base_overrides)
    overrides["eval_samples"] = eval_samples
    overrides["eval_samples_already_split_filtered"] = True
    return overrides
...
def main(argv=None):
    ...
    validation_base_overrides = _overrides_with_eval_samples(base_overrides, validation_eval_samples)
    final_base_overrides = _overrides_with_eval_samples(base_overrides, final_eval_samples)
    ...
    for index, params in enumerate(candidate_grid()):
        overrides = _overrides_with_eval_samples(base_overrides, validation_eval_samples)
    ...
    final_candidate_overrides = _overrides_with_eval_samples(base_overrides, final_eval_samples)
    ...
    report = {
        ...
        "experiment_note": (
            "Low/zero-PredReturn quick-profit replay derived from competing-risk "
            "fast-profit-vs-stop-first live attribution; replay-only and not live-switch evidence."
        ),
